...
function calCategoriesData(categories, opts, config, eachSpacing, context) {
  ...
  var categoriesTextLenth = categories.map(function (item) {
    return measureText(
      item,
      opts.xAxis.fontSize * opts.pix || config.fontSize,
      context
    );
  });
...
function getYAxisTextList(series, opts, config, stack, yData) {
  ...
  var minRange =
    yData.min === undefined || yData.min === null
      ...
      : yData.min;
  var maxRange =
    yData.max === undefined || yData.min === null
      ? dataRange.maxRange
      : yData.max;
  var range = [];
...
function calTooltipYAxisData(point, series, opts, config, eachSpacing) {
  ...
  for (let i = 0; i < ranges.length; i++) {
    ...
    item = opts.yAxis.data[i].formatter
      ? opts.yAxis.data[i].formatter(item)
      : item.toFixed(0);
...
function drawGaugeLabel(
  ...
  for (let i = 0; i < gaugeOption.splitLine.splitNumber + 1; i++) {
    ...
    var labelText = gaugeOption.formatter
      ? gaugeOption.formatter(nowNumber)
      : nowNumber;
    pos.x +=
...
function drawRadarLabel(
  ...
  angleList.forEach(function (angle, index) {
    ...
    if (util.approximatelyEqual(pos.x, 0)) {
      startX -=
        measureText(opts.categories[index] || "", config.fontSize, context) / 2;
    } else if (pos.x < 0) {
      startX -= measureText(
...
function drawMarkLine(opts, config, context) {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    context.setLineDash([]);
    if (item.showLabel) {
      let labelText = opts.yAxis.formatter
        ? opts.yAxis.formatter(item.value)
        : item.value;
      context.setFontSize(config.fontSize);
      let textWidth = measureText(labelText, config.fontSize, context);
        ...
        if (seriesIndex == 0) {
          for (let i = 0; i < points.length; i++) {
            ...
            if (item !== null && i > leftNum && i < rightNum) {
              ...
              if (columnOption.barBorderCircle) {
                var barBorderRadius =
                  (item.width - columnOption.meterBorder * 2) / 2;
                if (barBorderRadius > height) {
                  barBorderRadius = height;
                }
                context.moveTo(
                  startX + columnOption.meterBorder,
                  opts.height - opts.area[2]
                );
                context.lineTo(
                  startX + columnOption.meterBorder,
              ...
              if (columnOption.meterBorder > 0) {
                ...
                if (columnOption.barBorderCircle) {
                  var barBorderRadius =
                    (item.width - columnOption.meterBorder) / 2;
                  if (barBorderRadius > height) {
                    barBorderRadius = height;
                  }
                  context.moveTo(
    ...
    for (let i = 0; i < series.length; i++) {
      if (i == series.length - 1) {
        ...
        if (funnelOption.linearType == "custom") {
          ...
          grd.addColorStop(
            0,
            hexToRgb(series[i].color, funnelOption.fillOpacity)
          );
          grd.addColorStop(
            0.5,
            hexToRgb(
              funnelOption.customColor[series[i].linearIndex],
